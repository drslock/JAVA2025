...
**Objects** and **Classes** are fundamental building blocks of Object Oriented code that were introduced in the lecture session.
The slides and videos linked to above provide a recap of these concepts - what them again now if you need to refresh your memory.
...
- Open up the `Shapes` project that we gave you last week and find the `Triangle` class file (it should be somewhere in the `src/main/java` folder !)
- Add a constructor method to the `Triangle` class that takes three `int` parameters (which will be the lengths of each side of the triangle)
...
(For example, `toString` should return `This is a Triangle with sides of length 4, 5, 7`)
